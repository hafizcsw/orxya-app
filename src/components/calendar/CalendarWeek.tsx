...
export default function CalendarWeek({
  ...
  return <div className="w-full h-[calc(100vh-200px)] sm:h-[calc(100vh-160px)] flex flex-col bg-background rounded-lg overflow-hidden border border-border/20">
      ...
      <div className="hidden sm:flex items-center justify-between px-4 sm:px-6 py-2 border-b border-border/20">
        ...
        <button onClick={reload} disabled={loading} className="px-4 py-1.5 text-sm font-medium rounded-md hover:bg-accent/50 transition-colors disabled:opacity-50">
          {loading ? "..." : "تحديث"}
        </button>
      </div>
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex border-b border-border/10 bg-background h-10 sm:h-12 sticky top-0 z-20">
            ...
            {days.map((d, i) => {
              ...
              return <div key={i} className={cn("flex flex-col items-center justify-center gap-0.5 border-l border-border/30 text-center px-1", i === 0 && "border-l-0")}>
                  ...
                  onDateChange?.(d);
                  setGlobalDate(d);
                }} className={cn("flex items-center justify-center rounded-full transition-all font-medium cursor-pointer hover:bg-accent", "w-6 h-6 sm:w-7 sm:h-7 text-xs sm:text-sm", isToday ? "bg-[#1a73e8] text-white shadow-md hover:bg-[#1557b0]" : "text-foreground hover:bg-muted")}>
                    {d.getDate()}
                  </button>
                  </div>;
            })}
